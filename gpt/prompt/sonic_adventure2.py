...
本作は「ソニックチーム」からわずか11人がアメリカに派遣され、現地で5人増員して、その規模で開発していた。ソニックチームの面々は慣れない異国の地で開発に取り組んでいたため、飲みに行くどころか夕食も休みの日にどこかへ行くのにも皆一緒であり、まさに合宿状態であった。当時は少人数ながらもワンチームで2年に1本のペースで新作を出していくという使命のもと、アメリカで開発を続けていた^[\[5\]](#cite_note-dori-5)^。
...
ステージ終了時にはステージ中の行動で入ったステージスコアにクリアまでにかかった時間で決まるタイムボーナスが加算され、AからEまでの五段階の評価がなされる。ただし各ステージの2nd・3rdミッション、およびカートステージではタイムのみで評価が決まる他、ステージ内のリングを全て集められれば評価は必ずAランクになる^[\[6\]](#cite_note-6)^。
...
:   今作でエッグマンはアクションゲームの中では初のプレイヤーキャラクターとなった（アクションゲーム以外でなら、『ソニックドリフト{.mw-redirect}』が初）。ゲーム上の目的は、ステージ最後にあるゴールリングに触れることである。エッグマンはエッグモービルに逆関節の脚と武装を装着したメカに搭乗しており、ガンマのようなロックオンシューティングアクションを楽しむことができる。基本的な動作はテイルスと同様。また、体力の最大値がアップするアイテムが存在する（テイルスにはない）。チャオガーデンに限り、エッグモービルから降りた状態でプレイ可能。
...
ビッグ・ザ・キャット
:   声 - なし
:   ソニックアドベンチャーで初登場したのんびり屋の猫。今作はストーリーに関わらないが、DC版ではアクションステージのいたる箇所に登場している^[\[9\]](#cite_note-9)^。ドリームキャスト版のみ（ダークキャラとして）対戦モードで使用可能で、エッグマンと同様に2足歩行マシンに乗って戦う。また、ラストシナリオでのあるイベントシーンで特定のボタンを押すとビッグが出現する裏技がある。
...
:   初代『[ソニック・ザ・ヘッジホッグ](/wiki/%E3%82%BD%E3%83%8B%E3%83%83%E3%82%AF%E3%83%BB%E3%82%B6%E3%83%BB%E3%83%98%E3%83%83%E3%82%B8%E3%83%9B%E3%83%83%E3%82%B0_(1991%E5%B9%B4%E3%81%AE%E3%82%B2%E3%83%BC%E3%83%A0) "ソニック・ザ・ヘッジホッグ (1991年のゲーム)")』の同名ゾーンAct1を3Dアレンジしたステージ。急停止した際の効果音なども再現されている。ランク評価はされるが、クリアの特典は無く、記録もされない。
...
太古のメロディ（全キャラクター共通）
:   各ステージに必ずある遺跡^[\[10\]](#cite_note-10)^の近くでのみ使用可能で、使用すると別の道へのルートやアイテムが出現したりする。
...
ビッグフット（ソニック）
:   正式名称F-6t
    ビッグフット。二足歩行と飛行が可能であり、機銃での掃射やミサイルの発射などの攻撃をする。弱点はコックピットだが、普段は飛行しているため、着地の隙を狙う必要がある。
...
ホットショット（シャドウ）
:   正式名称B-3x
    ホットショット。ビッグフットの改良型。機銃の下にレーザー砲が追加されており、ロックオンレーザーを放つようになっている。弱点はビッグフットと同様コックピット。
...
フライングドッグ（ルージュ）
:   正式名称R-1/A
    フライングドッグ。ビッグフット･ホットショットの改良型。足が無く常に飛行しており、着地の隙を狙うことは不可能。攻撃手段･弱点はホットショットと同様。
...
:   ダメージを負ったバイオリザードが、エクリプスキャノンの砲口へとカオスコントロールでワープし、無理矢理アークを地球に落とそうとしている。本作のラストボスで宇宙空間での戦闘となり、ソニック、シャドウはスーパー化して立ち向かう。生命維持装置は既に壊されており、体にかかる大きな負担によって皮膚の一部が腫瘍の様に腫れ上がっており、そこに体当たりすることでのみダメージを与えられる。使用曲はCrush
    40作曲「Live & Learn」。
...
なお、\"ソニックアドベンチャー2
バトル\"と配信版の追加コンテンツでは新たに、育てたチャオ同士で戦わせる、\"チャオカラテ\"がプレイできる。こちらも優勝すると、エンブレムを入手できる。

キャラクターそのものとしてテイルス/エッグマンを操作できるのはこのエリアのみ。

ソニックアドベンチャー2バトルではゲームボーイアドバンスとジョイキャリーすることでゲームボーイアドバンスおよび同ハードのソフト『ソニックアドバンス』・『ソニックアドバンス2』にチャオを連れて行くことができ、チャオのプチガーデンが遊ぶことができる（『ソニックアドバンス』がなくてもチャオのプチガーデンだけでも遊ぶことは可能）。またメモリーカード間でチャオを交換する「ひっこし」も追加されている。過去にイベントやエンジョイプラスパックに同梱されていた限定ディスクで特別なチャオが配信されており、テイルス、ナックルズ、エミーを模したチャオが入手可能。
...
messages = [
    {
        ...
        "content": f"""
    あなたはゲーム配信者のリスナーです。
    あなたはゲーム配信者にゲームの内容に関して質問をしたり、コメントをしたりすることができます。
    ゲーム配信者はゲームをしながら、リスナーの質問やコメントに答えるのでゲーム配信者が理解しやすい言葉で短い文章を使った質問やコメントが好まれます。

    以下にゲーム配信者がプレイしているゲームの情報があります。この情報を使って、ゲーム配信者に質問やコメントをしてください。
    ...
    例:
    - ヒーローサイドのステージ1のシティエスケープは急勾配の坂道に作られた街を駆け下って行く、スピード感あふれるステージですね！
    - ヒーローサイドのステージ7のミッションストリートは昔苦戦した思い出があります。
    - ソニックかっこいいですね！
    - シャドウとソニックの掛け合いが好きです
    """,
    },
